...
def comp_duration(curve):
    ...
    df_aux['diff_richards'] = np.concatenate(([0], np.diff(curve.richards)), axis=0)
...
class EpiScanner:
    ...
    def _save_results(self, geocode, year, results, curve):
        ...
        self.curves[geocode].append({
                ...
                'sum_res': sum(abs(curve.richards - curve.casos_cum))/max(curve.casos_cum),  
    ...
    def plot_fit(self, geocode, year=0):
        ...
        for curve in self.curves[geocode]:
            ...
            axes[i].axvline(df.loc[df.index == pd.to_datetime(Week.fromstring(ep_duration['ini']).startdate()) ].index.values[0], color = 'black', ls = '--')
    
            axes[i].axvline(df.loc[df.index == pd.to_datetime(Week.fromstring(ep_duration['end']).startdate()) ].index.values[0], color = 'black', ls = '--', label='Epidemic \n period')
